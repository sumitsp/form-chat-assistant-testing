"""
Everest (Deephaven) matrix flow -> MySQL + Qdrant.

Separate entry point from Denali/Summit flows. Deephaven source content is spread
across multiple smaller matrix PDFs under a folder (not a single combined matrix PDF).

Vectors are upserted into the shared Qdrant collection ``mortgage_matrices`` and
distinguished by ``lender_id`` payload.

Usage:
  python ingest/lenders/everest_deephaven_flow_to_mysql_qdrant.py --dry-run
...
PROGRAM_REGISTRY: dict[str, dict[str, Any]] = {
    "EXPANDED PRIME": {
        "program_code": "DHM_EXPANDED_PRIME",
        "is_dscr_program": 0,
        "is_second_lien": 0,
        "is_itin_program": 0,
    ...
    "FOREIGN NATIONAL DSCR": {
        "program_code": "DHM_FOREIGN_NATIONAL_DSCR",
        "is_dscr_program": 1,
        "is_second_lien": 0,
        "is_itin_program": 0,
    ...
    "EQUITY ADVANTAGE": {
        "program_code": "DHM_EQUITY_ADVANTAGE",
        "is_dscr_program": 0,
        "is_second_lien": 0,
        "is_itin_program": 0,
        "is_foreign_national": 0,
    ...
    "EQUITY ADVANTAGE ELITE": {
        "program_code": "DHM_EQUITY_ADVANTAGE_ELITE",
        "is_dscr_program": 0,
        "is_second_lien": 0,
        "is_itin_program": 0,
        "is_foreign_national": 0,
    ...
    "EQUITY ADVANTAGE DSCR": {
        "program_code": "DHM_EQUITY_ADVANTAGE_DSCR",
        "is_dscr_program": 1,
        "is_second_lien": 0,
        "is_itin_program": 0,
        "is_foreign_national": 0,
    ...
    "EXPANDED PRIME SUPER JUMBO": {
        "program_code": "DHM_EXPANDED_PRIME_SUPER_JUMBO",
        "is_dscr_program": 0,
        "is_second_lien": 0,
        "is_itin_program": 0,
        "is_foreign_national": 0,
...
# Product banners (Deephaven matrices). Order matters: more specific patterns first.
_SECTION_HEADERS: list[tuple[re.Pattern[str], str]] = [
    (re.compile(r"^Expanded\s+Prime\s+Super\s+Jumbo\b", re.IGNORECASE), "EXPANDED PRIME SUPER JUMBO"),
    (re.compile(r"^Expanded\s+Prime\b", re.IGNORECASE), "EXPANDED PRIME"),
    (re.compile(r"^Non[-\s]*Prime\b", re.IGNORECASE), "NON-PRIME"),
    (re.compile(r"^ITIN\b", re.IGNORECASE), "ITIN"),
    (re.compile(r"^Foreign\s+National\b", re.IGNORECASE), "FOREIGN NATIONAL DSCR"),
    (re.compile(r"^DSCR\s*5[-\s]*9\s*Unit\b", re.IGNORECASE), "DSCR MULTI"),
    (re.compile(r"^DSCR\s+Multi\b", re.IGNORECASE), "DSCR MULTI"),
    (re.compile(r"^DSCR\b", re.IGNORECASE), "DSCR"),
    (re.compile(r"^BPL\b", re.IGNORECASE), "BPL"),
    (re.compile(r"^Equity\s+Advantage\s+Elite\b", re.IGNORECASE), "EQUITY ADVANTAGE ELITE"),
    (re.compile(r"^Equity\s+Advantage\s+DSCR\b", re.IGNORECASE), "EQUITY ADVANTAGE DSCR"),
    (re.compile(r"^Equity\s+Advantage\b", re.IGNORECASE), "EQUITY ADVANTAGE"),
]
...
def extract_deephaven_matrices_pdf(pdf_path: Path) -> list[dict[str, Any]]:
    ...
    for program_name, body, page_numbers in _iter_program_sections(pages):
        rows.append(
            {
                "investor_name": "Everest Deephaven",
    ...
    # Fallback for single-program PDFs where banner extraction is weak.
    name_up = pdf_path.name.upper()
    inferred_program = None
    if "BPL" in name_up:
        inferred_program = "BPL"
    elif "5-9" in name_up or "MULTI" in name_up:
        inferred_program = "DSCR MULTI"
    elif "EQUITY" in name_up and "DSCR" in name_up:
        inferred_program = "EQUITY ADVANTAGE DSCR"
    elif "EQUITY" in name_up and "ELITE" in name_up:
        inferred_program = "EQUITY ADVANTAGE ELITE"
    elif "EQUITY" in name_up:
        inferred_program = "EQUITY ADVANTAGE"
    elif "ITIN" in name_up:
        inferred_program = "ITIN"
    elif "NON" in name_up and "PRIME" in name_up:
        inferred_program = "NON-PRIME"
    elif "EXPANDED" in name_up and "PRIME" in name_up:
        inferred_program = "EXPANDED PRIME"
    elif "DSCR" in name_up:
        inferred_program = "DSCR"
    if inferred_program:
        body = _normalize_whitespace("\n\n".join(pages))
        rows.append(
            {
                "investor_name": "Everest Deephaven",
                "program_name": inferred_program,
                "rules_common_all_programs": common,
                "program_matrices_and_rules": body,
...
def _discover_matrix_pdfs(input_path: Path) -> list[Path]:
    ...
    # Skip pricing matrix files that are not underwriting matrix content.
    files = sorted(p for p in input_path.glob("*.pdf") if "PPP" not in p.name.upper())
    if not files:
        raise RuntimeError(f"No PDF files found under {input_path}")
    return files


def extract_deephaven_matrices(input_path: Path) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for pdf in _discover_matrix_pdfs(input_path):
        parsed = extract_deephaven_matrices_pdf(pdf)
        if parsed:
            rows.extend(parsed)
    return rows
...
def _extract_program_loan_bounds(body: str, *, default_min: int, default_max: int) -> tuple[int, int]:
    ...
    # Deephaven usually prints min/max under GENERAL REQUIREMENTS.
...
def _sanitize_property_type_value(raw: str | None, max_len: int = 280) -> str | None:
    ...
    work = re.split(r"\bINELIGIBLE\b", work, flags=re.IGNORECASE, maxsplit=1)[0]
    work = re.sub(r"\b(?:MAX|OVERLAY|VERLAY|PURCHASE|REFINANCE|LOAN AMOUNT|OTHER|REQUIREMENTS?)\b.*", "", work, flags=re.I)
    work = re.sub(r"\s+and\s+", ", ", work, flags=re.I).strip(" ,;")
    if not work:
        return None

    # Canonical labels for SQL storage.
    patterns: list[tuple[str, re.Pattern[str]]] = [
        ("SFR", re.compile(r"\b(SFR|SINGLE\s+FAMILY)\b", re.I)),
        ("PUD", re.compile(r"\bPUD\b", re.I)),
        ("Townhome", re.compile(r"\bTOWNHOMES?\b", re.I)),
        ("Condominium", re.compile(r"\b(CONDO|CONDOMINIUM)\b", re.I)),
        ("Non-Warrantable Condo", re.compile(r"\b(NW\s*CONDO|NON[-\s]*WARRANTABLE\s+CONDO)\b", re.I)),
        ("2-4 Unit", re.compile(r"\b2\s*-\s*4\s*UNITS?\b", re.I)),
        ("5-9 Unit", re.compile(r"\b5\s*-\s*9\s*UNITS?\b", re.I)),
        ("5-8 Unit", re.compile(r"\b5\s*-\s*8\s*UNITS?\b", re.I)),
        ("Co-op", re.compile(r"\bCO[-\s]*OPS?\b", re.I)),
        ("Manufactured", re.compile(r"\bMANUFACTURED\b", re.I)),
        ("Rural", re.compile(r"\bRURAL\b", re.I)),
        ("Condotel", re.compile(r"\bCONDOTELS?\b", re.I)),
    ]
    out: list[str] = []
...
def _extract_property_type_from_table(tbl: list[list[Any]]) -> str | None:
    ...
    for ln in lines:
        ...
        if "PROPERTY TYPE" in uc:
            collecting = True
            # Capture inline values like: "Property Types SFR, PUD, Condo"
            inline = re.split(r"PROPERTY\s+TYPES?\s*:?\s*", ln, flags=re.IGNORECASE, maxsplit=1)
            if len(inline) == 2 and inline[1].strip():
                pnorm = _sanitize_property_type_value(_normalize_property_type(inline[1].strip()))
                if pnorm:
                    props.append(pnorm)
            continue
        if not collecting:
...
def _extract_property_type_from_page_text(page_text: str) -> str | None:
    ...
    for ln in lines:
        ...
        if "PROPERTY TYPE" in uc:
            ...
            inline = re.split(r"PROPERTY\s+TYPES?\s*:?\s*", ln, flags=re.IGNORECASE, maxsplit=1)
            if len(inline) == 2 and inline[1].strip():
                norm = _sanitize_property_type_value(_normalize_property_type(inline[1].strip()))
                if norm:
                    props.append(norm)
            continue
        if not collecting:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page_num in page_numbers:
            ...
            for tbl in tables:
                ...
                has_matrix_markers = bool(
                    re.search(r"(MAX\s+LTV|MAX\s+CLTV|LTV/CLTV|LTV\s*/\s*CLTV)", flat_table_text, re.IGNORECASE)
                    or re.search(r"ELIGIBILITY\s+MATRIX", flat_table_text, re.IGNORECASE)
                    or (
                        re.search(r"DSCR", flat_table_text, re.IGNORECASE)
                        and re.search(r"FICO", flat_table_text, re.IGNORECASE)
                        and re.search(r"LOAN\s+AMOUNT", flat_table_text, re.IGNORECASE)
                    )
                )
                if not has_matrix_markers:
                    continue
                if not re.search(r"LOAN\s+AMOUNT", flat_table_text, re.IGNORECASE):
                    continue
                if not re.search(r"\bFICO\b", flat_table_text, re.IGNORECASE):
                    continue
                ...
                has_dscr_split_cols = bool(
                    (
                        re.search(r"DSCR\s*[<≤]\s*1(?:\.00)?", flat_table_text, re.IGNORECASE)
                        or re.search(r"DSCR\s*<\s*1(?:\.00)?", flat_table_text, re.IGNORECASE)
                    )
                    and (
                        re.search(r"DSCR\s*[≥>]\s*1(?:\.00)?", flat_table_text, re.IGNORECASE)
                        or re.search(r"DSCR\s*>\s*=\s*1(?:\.00)?", flat_table_text, re.IGNORECASE)
                        or re.search(r"MINIMUM\s+1(?:\.00)?", flat_table_text, re.IGNORECASE)
                    )
                )
                ...
                for r in tbl:
                    ...
                    def add_row(
                        ...
                        *,
                        ...
                        dscr_band_override: str | None = None,
                    ) -> None:
                        if p is None:
                            return
                        rows.append(
                            {
                                ...
                                "doc_type": doc_type,
                                "dscr_band": dscr_band_override or current_dscr_band,
                    ...
                    if has_dscr_split_cols and len(numeric_after) >= 4:
                        gte_purchase = numeric_after[0]
                        gte_cash = numeric_after[1]
                        lt_purchase = numeric_after[2]
                        lt_cash = numeric_after[3]
                        add_row(
                            "any",
                            gte_purchase,
                            gte_purchase,
                            gte_cash,
                            dscr_band_override="gte_1_00",
                        )
                        add_row(
                            "any",
                            lt_purchase,
                            lt_purchase,
                            lt_cash,
                            dscr_band_override="gte_0_75_lt_1_00",
                        )
                        continue
...
def _replace_ltv_matrix(
    ...
    if len(rows) < 2:
        print(
            f"Warning: LTV parse coverage too low for {program_name} from {pdf_path.name} "
            f"(rows={len(rows)}). Skipping LTV inserts for this section."
        )
        return 0
    inserted = 0
        ...
        if row.get("max_ltv_rate_term") is not None:
            conn.execute(
                ...
                ),
                {
                    ...
                    "max_cltv": row["max_ltv_rate_term"],
                    "ltv_note": ("table-first parsed from matrix; " + row.get("track_note", "")).strip("; "),
                },
            )
            inserted += 1
...
def create_matrix_qdrant_collection() -> None:
    """Create shared matrix collection + indexes if missing."""
    ...
    print("Note: use --apply to add Deephaven lender vectors.")


def _remove_lender_vectors_from_shared_collection(client: QdrantClient) -> None:
    """Delete only this lender's points from the shared matrix collection."""
...
def _upsert_qdrant_chunks(
    ...
    _remove_lender_vectors_from_shared_collection(client)
    _ensure_qdrant_collection(client, QDRANT_COLLECTION)
    ...
    common_parts = [
        _normalize_whitespace(str(r.get("rules_common_all_programs", "")))
        for r in rows
        if str(r.get("rules_common_all_programs", "")).strip()
    ]
    common = "\n\n".join(dict.fromkeys(common_parts))
    if common.strip():
        ...
        source_files = sorted({str(r.get("source_file", "")) for r in rows if str(r.get("source_file", "")).strip()})
        points.append(
            models.PointStruct(
...
def run(input_path: Path, do_apply: bool) -> None:
    rows = extract_deephaven_matrices(input_path)
    if not rows:
        raise RuntimeError("No Deephaven program rows parsed from input.")

    if not do_apply:
        source_files = sorted({str(r.get("source_file", "")) for r in rows if str(r.get("source_file", "")).strip()})
        print(f"Dry run: parsed {len(rows)} program sections from {len(source_files)} PDF(s).")
        print("Source PDFs:", ", ".join(source_files))
        print("Programs:", ", ".join(dict.fromkeys(r["program_name"] for r in rows)))
        return
    ...
    with engine.begin() as conn:
        ...
        for row in rows:
            ...
            source_pdf = Path(str(row.get("source_pdf", input_path)))
            ltv_inserted += _replace_ltv_matrix(conn, lender_id, pid, pname, source_pdf, row.get("page_numbers", []))
            _sync_program_allowed_fields(
                conn,
                lender_id,
                pid,
                body,
                doc_from_income_section=False,
            )
    ...
    with engine.begin() as conn:
        programs = conn.execute(
            text(
                ...
                "WHERE program_code LIKE 'DHM_%' ORDER BY program_id"
            )
        ).fetchall()
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Everest (Deephaven) matrix ETL to MySQL + Qdrant (shared matrix collection)")
    ap.add_argument(
        "--input-path",
        type=Path,
        default=PDFS_DEFAULT_DIR,
        help="Path to Deephaven matrix PDF file OR folder containing matrix PDFs",
    )
    ...
    ap.add_argument("--audit-sql", action="store_true", help="Print SQL audit summary for loaded DHM rows")
    ap.add_argument(
        "--init-qdrant",
        action="store_true",
        help=(
    ...
    if args.init_qdrant:
        ...
        create_matrix_qdrant_collection()
        return
    if args.audit_sql:
        audit_sql()
        return
