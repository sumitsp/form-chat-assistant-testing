"""
Summit (Verus) VMC — Seller guideline PDF → Qdrant (hierarchical TOC chunks).
...
Collection: Summit_Versus_mortgage_guideline

Usage:
  python ingest/lenders/summit_verus_guidelines_qdrant.py           # preview only (no Qdrant)
  python ingest/lenders/summit_verus_guidelines_qdrant.py --dry-run  # same
  python ingest/lenders/summit_verus_guidelines_qdrant.py --apply # embed + upsert to Qdrant
"""
...
def _default_pdf_path() -> Path:
    candidates = [
        ...
        / "Summit (Verus)"
        / "Guidelines"
        / "VMC-Correspondent-Non-Agency-Seller-Guide-03.09.2026-Final.pdf",
        ROOT
        / "input"
        / "input"
        / "Summit (Verus)"
...
PDF_DEFAULT = _default_pdf_path()
COLLECTION_NAME = "Summit_Versus_mortgage_guideline"
LENDER_CODE = "VMC"
...
def upsert_chunks(
    ...
    *,
    ...
    for i, chunk in enumerate(chunks):
        ...
        cid = str(uuid.uuid5(uuid.NAMESPACE_URL, f"vmc-guideline::{pdf_name}::{chunk['section_path']}"))
        ...
        payload: dict[str, Any] = {
            ...
            "doc_kind": "summit_verus_underwriting_guideline",
        }
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Summit (Verus) underwriting guidelines PDF → Qdrant (TOC-aware chunks)")
